...
def create_blemish_overlay(image, blemish_mask):
    """
    Overlay blemish regions in purple/magenta on the original image.
    """
...
def detect_fruit_blemish(
    ...
    # ========================================================
    # Add black border so flood fill always starts from background
    ...
    internal_holes = cv2.bitwise_not(flood_filled)

    # Add internal holes back into fruit
    filled_mask = cv2.bitwise_or(padded_mask, internal_holes)

    # Remove temporary border
    fruit_mask_binary = filled_mask[1:-1, 1:-1]

    # Slightly shrink fruit mask to avoid dark boundary pixels
    # being detected as blemishes
    erosion_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    fruit_mask_binary = cv2.erode(fruit_mask_binary, erosion_kernel, iterations=1)

    hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    hsv_ranges = get_fruit_specific_hsv_ranges(fruit_type)

    combined_mask = np.zeros(fruit_mask_binary.shape, dtype=np.uint8)
    ...
    blemish_mask = cv2.bitwise_and(combined_mask, fruit_mask_binary)
    ...
    blemish_mask = cv2.morphologyEx(blemish_mask, cv2.MORPH_OPEN, opening_kernel)
    blemish_mask = cv2.morphologyEx(blemish_mask, cv2.MORPH_CLOSE, closing_kernel)

    # Remove tiny noise
    blemish_mask = remove_small_components(blemish_mask, min_area=min_component_area)

    fruit_pixels = cv2.countNonZero(fruit_mask_binary)
    blemish_pixels = cv2.countNonZero(blemish_mask)

    if fruit_pixels > 0:
        blemish_percentage = (blemish_pixels / fruit_pixels) * 100
    else:
        blemish_percentage = 0.0

    overlay_image = create_blemish_overlay(image, blemish_mask)
